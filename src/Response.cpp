...
void   Response::contentType()
{
    ...
    if(_target_file.rfind(".", STR::npos) != STR::npos && _code == 200)
        _response_content.append(_mime.getMimeType(_target_file.substr(_target_file.rfind(".", STR::npos))) );
    else
...
static bool fileExists (const STR& f) {
...
static bool    checkReturn(Location &loc, short &code, STR &location)
{
...
static STR combinePaths(STR p1, STR p2, STR p3)
{
    STR res;
    int         len1;
    int         len2;
...
int Response::handleCgiTemp(STR &location_key)
{
    STR path;
    path = _target_file;
    _cgi_obj.clear();
    ...
    _cgi_obj.EnvCgiInitialization(request, _server.getLocationKey(location_key)); // + URI
    _cgi_obj.execute(this->_code);
    return (0);
}

int        Response::handleCgi(STR &location_key)
{
    STR path;
    STR exten;
    size_t      pos;
    ...
    if (ConfigurationFile::checkFile(path, 1) == -1 || ConfigurationFile::checkFile(path, 3) == -1)
    {
        _code = 403;
        return (1);
    }
    ...
    _cgi_obj.EnvInitialization(request, _server.getLocationKey(location_key)); // + URI
    _cgi_obj.execute(this->_code);
    return (0);
}

static void    getLocationMatch(STR &path, std::vector<Location> locations, STR &location_key)
{

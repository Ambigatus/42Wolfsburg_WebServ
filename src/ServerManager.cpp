...
void    ServerManager::runServers()
{
    ...
    while (true)
    {
        timer.tv_sec = 1;
        timer.tv_usec = 0;
        recv_set_cpy = _read_fd_set;
        write_set_cpy = _write_fd_set;
...
void    ServerManager::initializeSets()
{
    ...
    for(std::vector<ServerConfiguration>::iterator it = _servers.begin(); it != _servers.end(); ++it)
    {
        ...
        addToSet(it->getFd(), _read_fd_set);
        _servers_map.insert(std::make_pair(it->getFd(), *it));
    }
    // at this stage _max_fd will belong to the last server created.
    _max_fd = _servers.back().getFd();
}

void    ServerManager::acceptNewConnection(ServerConfiguration &serv)
{
...
void    ServerManager::closeConnection(const int i)
{
    if (FD_ISSET(i, &_write_fd_set))
        removeFromSet(i, _write_fd_set);
    if (FD_ISSET(i, &_read_fd_set))
        removeFromSet(i, _read_fd_set);
    close(i);
...
{
    int bytes_sent;
    std::string response = c.response.getResult();
...
{
    for (std::vector<ServerConfiguration>::iterator it = _servers.begin();
        it != _servers.end(); ++it)
    {
...
void    ServerManager::readRequest(const int &i, Client &c)
{
    ...
    {
        c.updateTime();
        c.request.methodCheck(buffer, bytes_read);
    ...
    if (c.request.parsingCompleted() || c.request.errorCode()) // 1 = parsing completed and we can work on the response.
    {
        ...
        {
            handleReqBody(c);
            addToSet(c.response._cgi_obj.pipe_in[1], _write_fd_set);
            addToSet(c.response._cgi_obj.pipe_out[0], _read_fd_set);
        }
        removeFromSet(i, _read_fd_set);
        addToSet(i, _write_fd_set);
    }
}
...
void    ServerManager::sendCgiBody(Client &c, CGIConfig &cgi)
{
    int bytes_sent;
    ...
    if (bytes_sent < 0)
    {
        ...
        removeFromSet(cgi.pipe_in[1], _write_fd_set);
        close(cgi.pipe_in[1]);
...
void    ServerManager::readCgiResponse(Client &c, CGIConfig &cgi)
{
    ...
    if (bytes_read == 0)
    {
        removeFromSet(cgi.pipe_out[0], _read_fd_set);
        close(cgi.pipe_in[0]);
